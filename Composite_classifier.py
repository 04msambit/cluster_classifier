...
term_dict_business_frequency=collections.defaultdict(dict)
term_dict_politics_frequency=collections.defaultdict(dict)
term_dict_entertainment_frequency=collections.defaultdict(dict)
...
def naive_bays_classification(fileName1,fileName2,fileName3,fileName4,fileName5,fileName6):
    ...
    value_list=[]
    for key in term_dict_entertainment:
        value_list.append(term_dict_entertainment[key])

    value_list.sort(reverse=True)

    length= int ( 0.85 * len(value_list))
    counter = 0
    term_dict_entertainment_frequency={}
    for i in range(0,length):
        search_value=value_list[i]
        for keys in term_dict_entertainment:
            if ( term_dict_entertainment[keys] == search_value ):
                if ( counter < length):
                   if keys not in term_dict_entertainment_frequency:
                      term_dict_entertainment_frequency[keys] = search_value
                      counter+=1
    ...
    # Frequency Dictionary for Feature Selection
    
    value_list=[]
    for key in term_dict_business:
        value_list.append(term_dict_business[key])

    value_list.sort(reverse=True)

    length= int ( 0.85 * len(value_list))
    counter=0
    term_dict_business_frequency={}
    for i in range(0,length):
        search_value=value_list[i]
        for keys in term_dict_business:
            if ( term_dict_business[keys] == search_value ):
                if (counter < length):
                   if keys not in term_dict_business_frequency:
                      term_dict_business_frequency[keys] = search_value
	              counter+=1
    ...
    value_list=[]
    for key in term_dict_politics:
        value_list.append(term_dict_politics[key])
    ...
    length= int ( 0.85 * len(value_list))
    counter = 0
    term_dict_politics_frequency={}
    for i in range(0,length):
        search_value=value_list[i]
        for keys in term_dict_politics:
            if ( term_dict_politics[keys] == search_value ):
                if ( counter < length ):
                   if keys not in term_dict_politics_frequency:
                   	term_dict_politics_frequency[keys] = search_value
...
def calculate_class(word_list,ttl,exp_class):
    ...
    for each_term in word_list:
        ...
        denominator = total_terms_entertainment_class + len(term_dict_entertainment_frequency)
        ...
        denominator = total_terms_business_class + len(term_dict_business_frequency)
        frac = float(numerator) / denominator
        log_form = float(math.log(frac,2))
        ...
        if each_term in term_dict_politics:
            ...
            numerator = 1
        denominator = total_terms_politics_class + len(term_dict_politics_frequency)
        frac = float(numerator) / denominator
        log_form = float(math.log(frac,2))
